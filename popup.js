...
function getSolutionElement(solutionInfo) {
    ...
    if(!!imgMatchs) {
        for(match of imgMatchs) {
            ...
            let imgIdx = solutionInfo.questionInfo.questionResourceList.findIndex((e) => {
                return e.imgNo === parseInt(imgNum);
            })
            let imgInfo = solutionInfo.questionInfo.questionResourceList[imgIdx];
...
async function getCookies(domain, name) {
    ...
    if(navigator.userAgent.includes("Firefox/")) 
        cookie_val = await browser.cookies.get({"url": domain, "name": name}); // firefox
    else
        cookie_val = await chrome.cookies.get({"url": domain, "name": name}); // chrome
...
async function getAccessToken() {
    let accessToken = await getCookies('https://ai.matamath.net', 'token_access');
    return accessToken;
}

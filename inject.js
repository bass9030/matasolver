...
async function checkLoad() {
    let buttons = document.querySelectorAll('section > div:nth-child(1) > div > button');
    if(!!!buttons.length) {
    ...
    buttons.forEach(button => {
        button.addEventListener('click', async (event) => {
            ...
            let solutionInfo = await parseSolutionInfo(await getSolution(questionId));
            // console.log(solutionInfo);
            let questionDiv = document.querySelector('section > div:nth-child(2) > div > div > div > div > div.swiper-no-swiping > div > div');
            let detailsEl = document.querySelector('section > div:nth-child(2) > div > div > div > div > div.swiper-no-swiping > div > div > details');
